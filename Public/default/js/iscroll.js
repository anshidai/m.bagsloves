...
var m = Math,
	...
	iScroll = function (el, options) {
		...
		//to some margin top offset
		that.scrollTop = parseInt($(that.scroller).css('margin-top'));

		// Set starting position
		that.x = that.options.x;
		that.y = that.options.y || that.scrollTop;
...
iScroll.prototype = {
	...
	_resize: function () {
		var that = this;
		setTimeout(function () { that.destroy(); }, isAndroid ? 200 : 0);
	},
	...
	_end: function (e) {
		...
		if (that.zoomed) {
			...
			if(scale == that.options.zoomMin){
				$(that.scroller).css('margin-top',that.scrollTop);
			}else{
				$(that.scroller).css('margin-top','0');
			}
