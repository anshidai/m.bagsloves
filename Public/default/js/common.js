...
{
    return $isvalidemail=/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))$/i.test(email);
}
...
var userFastLogin = function() {
    return {
        exec: function(callback) {
            ...
            $.ajax({
                url: '/index.php?m=wap&c=user&a=isLoginAjax',
                type: 'GET',
                dataType: 'json',
                cache: false,
                async: false,
                success: function(retdat) {
                    if (retdat.status == 1) {
                        ...
                        $.Prompt('Please login After.',3);
//                        if (userFastLoginDialog == null) {
//                            $.ajax({
//                                url: '/index.php?m=m&c=user&a=loginAjax',
//                                type: 'GET',
//                                dataType: 'html',
//                                cache: false,
//                                async: false,
...
$.fn.newMobileSlide = function(options) {
    function testTransform3d() {
        var supported = false;
        var div = $('<div style="position:relative">Transform Test</div>');
        $('body').append(div);
        div.css({
    ...
    $.extend($.touchSlider.prototype, {
        ...
        init: function() {
            ...
            if (this.loop && (this.len / this.visible > 1)) {
                if (!this.resize) {
        ...
        update: function() {
            ...
            if (triggers && triggers[curIndex]) {
                this.triggerSel && ($(this.triggerSel).removeClass(cls));
                $(triggers[curIndex]).addClass(cls);
    ...
    new $.touchSlider(this, options)
};
$.zoomImgLayer = function(widthEl, heightEl) {
    if ((widthEl / heightEl) > parseFloat(window.innerWidth / window.innerHeight)) {
        ...
        $('.zoom-img').width(window.innerWidth + 'px');
        $('.zoom-img').height('auto');
        $('.zoom-img').css('margin-top', padTop + 'px')
    } else {
        $('.zoom-img').width('100%');
